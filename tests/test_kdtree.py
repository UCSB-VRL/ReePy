...
from reepy.kdtree import KDTree

def test_kdtree_functionality():
    dim = 3
    ...
    # Assert that query results match
    assert kd_tree_results == naive_results, "Query results mismatch"
    
    # Assert that iterator returns correct number of points
    assert len(list(kd_tree)) == len(points) + len(additional_points), \
