...
def test_random():
    ...
    for u, v in reeb.edges():
        ...
        assert not src_trajs.isdisjoint(dst_trajs), (
            f"No intersection between {u} and {v}"
        )


def test_random_base():
    # 33 days with a 10 second SR for 2D points
    points = np.random.rand(100, 100, 2)
    time = np.arange(points.shape[0])[None, :, None].repeat(points.shape[0], axis=0)
    data = np.concatenate([time, points], axis=2)

    orderer = lambda x: int(x[0])
    equivalence = lambda x, y: (np.linalg.norm(x[1:] - y[1:]) < 0.1)

    reeb = reepy.ReebGraph(orderer=orderer, equivalence=equivalence)
    reeb.append_trajectories(data)
